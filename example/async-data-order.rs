...
//
// A server-streaming handler sends FRAMES_PER_STREAM DATA frames (seq 0..49)
// per stream. The client opens ITERATIONS concurrent streams on a
// multi-threaded runtime and asserts that every stream receives its DATA
// frames in sequence order. With the old spawn-per-frame design, that many
// concurrent streams create enough task-list separation between same-stream
// DATA frames to trigger reordering.
...
async fn run() {
    ...
    for _ in 0..ITERATIONS {
        ...
        handles.push(tokio::spawn(async move {
            ...
            assert_eq!(
                expected_seq, FRAMES_PER_STREAM,
                "stream ended after {expected_seq} frames"
            );
            if saw_reorder {
