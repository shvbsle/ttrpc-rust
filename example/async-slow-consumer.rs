...
// frames and then never reads it, so those frames pile up on the client's
// per-stream queue. While the stream sits unread, the client issues an
// unrelated unary RPC on the same connection and asserts it still completes.
//
...
const EXPECTED_BUFFERED_FRAMES: u32 = 100;
...
async fn run() {
    ...
    // Frames accepted before overflow remain ordered and readable. Once they
    // are drained, this stream alone reports the terminal overflow error.
    let buffered = tokio::time::timeout(RPC_TIMEOUT, async {
        let mut expected_seq = 0;
        loop {
            match unread.recv().await {
                Ok(Some(payload)) => {
                    assert_eq!(payload.seq, expected_seq);
                    expected_seq += 1;
                }
                Ok(None) => panic!("overflowing stream ended without an error"),
                Err(e) => {
                    assert!(
                        e.to_string()
                            .contains("stream inbound queue capacity exceeded"),
    ...
    .await
    .expect("timed out draining the overflowing stream");
    assert_eq!(buffered, EXPECTED_BUFFERED_FRAMES);

    server.shutdown().await.unwrap();

    eprintln!("unary RPC completed and the slow stream failed after buffering {buffered} frames");
}
